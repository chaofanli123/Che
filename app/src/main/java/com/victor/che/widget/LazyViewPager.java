...
/**
 * 不会提前预加载界面的ViewPager，是对androidViewPager代码的改写
 * 
 * @author Victor
 * @email 468034043@qq.com
 * @time 2016年10月30日 下午6:39:33
 */
public class LazyViewPager extends ViewGroup {
	 ...
	 * {@link com.bxg.news.view.LazyViewPager.OnPageChangeListener} interface
	 * with stub implementations of each method. Extend this if you do not
	 * intend to override every method of
	...
	public void setOffscreenPageLimit(int limit) {
		if (limit < DEFAULT_OFFSCREEN_PAGES) {
			Log.w(TAG,
					"Requested offscreen page limit " + limit + " too small; defaulting to " + DEFAULT_OFFSCREEN_PAGES);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mChildWidthMeasureSpec = MeasureSpec.makeMeasureSpec(getMeasuredWidth() - getPaddingLeft() - getPaddingRight(),
				MeasureSpec.EXACTLY);
	...
	public void fakeDragBy(float xOffset) {
		...
		final long time = SystemClock.uptimeMillis();
		final MotionEvent ev = MotionEvent.obtain(mFakeDragBeginTime, time, MotionEvent.ACTION_MOVE, mLastMotionX, 0,
				0);
		mVelocityTracker.addMovement(ev);
		ev.recycle();
	}
