...
import com.victor.che.bean.XunJianJiLu;
...
public class XunJianJiLuActivity extends BaseActivity {
    ...
    private List<XunJianJiLu.PageBean.ListBean> messageArrayList;
    private PtrHelper<XunJianJiLu.PageBean.ListBean> mPtrHelper;
    private int index;/*点击的愿望下标*/
    public static int currentPos = 0;//当前位置
    @Override
    public int getContentView() {
        return R.layout.activity_xun_jian_ji_lu;
    }
    ...
    private void loadData(final boolean pullToRefresh, int curpage, final int pageSize) {
        ...
        VictorHttpUtil.doPost(mContext, Define.URL_XunJianJiLu + ";JSESSIONID=" + MyApplication.getUser().JSESSIONID, params, false, null,
                new BaseHttpCallbackListener<Element>() {
                    @Override
                    public void callbackSuccess(String url, Element element) {
                        XunJianJiLu Policy = JSON.parseObject(element.body, XunJianJiLu.class);
                        //                        List<QueryUserCarHistory> queryUserCarHistories = new ArrayList<QueryUserCarHistory>();
                        List<XunJianJiLu.PageBean.ListBean> shopsCouponList = new ArrayList<>();
     ...
     * 订单列表适配器
     */
    private class CouponAdapter extends QuickAdapter<XunJianJiLu.PageBean.ListBean> {

        public CouponAdapter(int layoutResId, List<XunJianJiLu.PageBean.ListBean> data) {
            super(layoutResId, data);
        }

        @Override
        protected void convert(BaseViewHolder holder, final XunJianJiLu.PageBean.ListBean shopsCoupon) {
