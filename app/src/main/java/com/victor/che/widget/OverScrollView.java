...
public class OverScrollView extends FrameLayout implements OnTouchListener {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (activePointerId == INVALID_POINTER) {
				// If we don't have a valid car_brand_series_id, the touch down wasn't on
				// content.
				break;
			}
	...
	public boolean isOverScrolled() {
		return (getScrollY() < child.getPaddingTop()
				|| getScrollY() > child.getBottom() - child.getPaddingBottom() - getHeight());
	}
